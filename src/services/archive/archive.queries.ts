...
import { archiveKeys } from './archive.keys';
...
export const useGuestArchiveListQuery = (
  ...
  return useQuery({
    ...
    select: (response: ApiResponse<PageResponse<ArchiveResponse.ArchiveItems>>) => response.data,
    staleTime: 1000 * 60 * 5,
    enabled: !!nickname,
  });
};
...
export const useGuestArchivePostsQuery = (
  ...
  return useQuery({
    ...
    select: (response: ApiResponse<PageResponse<ArchiveResponse.PostItems>>) => response.data,
    staleTime: 1000 * 60 * 5,
    enabled: !!archiveId,
  });
};
